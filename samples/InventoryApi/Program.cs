...
await using (var scope = app.Services.CreateAsyncScope())
{
    ...
    if (!await db.Products.AnyAsync())
    {
        db.Products.AddRange(
            ...
            new Product { Name = "Gizmo",   Price = 4.99m,  Stock = 200 });
...
app.MapGet("/products/{id:int}/price", async (int id, InventoryDbContext db) =>
{
    ...
    return product is null ? Results.NotFound() : Results.Ok(new { product.Id, product.Name, product.Price });
});
...
app.MapPost("/products/{id:int}/reserve", async (int id, int qty, InventoryDbContext db) =>
{
    await using var tx = await db.Database.BeginTransactionAsync();
    ...
    return Results.Ok(new { product.Id, product.Name, reserved = qty, remaining = product.Stock });
});
...
app.MapPut("/products/{id:int}/price", async (int id, decimal newPrice, InventoryDbContext db) =>
{
    if (newPrice <= 0)
...
app.MapPost("/products/{id:int}/reserve-with-timeout", async (int id, int qty, InventoryDbContext db) =>
{
    await using var tx = await db.Database.BeginTransactionAsync();
