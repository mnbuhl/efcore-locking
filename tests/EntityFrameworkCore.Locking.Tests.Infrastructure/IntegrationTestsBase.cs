...
public abstract partial class IntegrationTestsBase : IAsyncLifetime
{
    ...
    protected abstract (TestDbContext ctx, SqlCapture capture) CreateContextWithCapture();

    protected abstract Task ResetDatabaseAsync(TestDbContext ctx);

    protected static async Task<(int categoryId, int productId)> SeedAsync(
        TestDbContext ctx,
